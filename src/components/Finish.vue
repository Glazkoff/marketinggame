        ...
        <div class="col-6 "  v-if="isSecondPosition">
          <div class="card mb-3" style="max-width: 540px;">
            <div class="row no-gutters">
              ...
              <div class="col-md-9">
                <div class="card-body">
                  <h5 class="card-title" v-if="isWinner">Второе место - <strong>{{secondPosition.name}}</strong></h5>
                  <h5 class="card-title" v-if="isPrizer">Первое место - <strong>{{secondPosition.name}}</strong></h5>
                  <p class="card-text">Игроку удалось заработать <br><em>{{secondPosition.money}} ₽</em></p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="col-6" v-if="isThirdPosition">
          <div class="card mb-3" style="max-width: 540px;">
            <div class="row no-gutters">
              ...
              <div class="col-md-9">
                <div class="card-body">
                     <h5 class="card-title" v-if="isWinner || isSecondWinner">Третье место - <strong>{{thirdPosition.name}}</strong></h5>
                  <h5 class="card-title" v-if="isThirdWinner">Второе место - <strong>{{thirdPosition.name}}</strong></h5>
                  <p class="card-text">Игроку удалось заработать <br><em>{{thirdPosition.money}} ₽</em></p>
                </div>
              </div>
            </div>
          </div>
         ...
         <div class="row">
             <div class="col-6 align-content-center justify-content-between">
                 <router-link to="Choose" tag="button" class="btn btn-info w-100" style="margin: auto auto; display: block;">Cоздать новую комнату</router-link>
             </div>
             <div class="col-6">
                 <button class="btn btn-secondary w-100" style="margin: auto auto; display: block;" disabled>Играть ещё раз в комнате</button>
             </div>
         </div>
    </div>
  </div>
...
<script>
  export default {
    ...
    computed: {
        ...
        isPrizer() {
                return ((this.$socket.id===this.$store.state.winners[2].id)||(this.$socket.id===this.$store.state.winners[3].id))
        },
        gamerData() {
            if (this.isWinner) {
        ...
        secondPosition() {
            if (this.$store.state.winners.hasOwnProperty('2')) {
                if (this.isWinner) {
                    return this.$store.state.winners[2];
                } else if (this.$socket.id===this.$store.state.winners[2].id || this.$socket.id===this.$store.state.winners[3].id) {
                    return this.$store.state.winners[1];
                } 
            } else return false;
        },
        thirdPosition() {
            if (this.$store.state.winners.hasOwnProperty('3')) {
                if (this.isWinner || this.$socket.id===this.$store.state.winners[2].id) {
                    return this.$store.state.winners[3];
                } else if (this.$socket.id===this.$store.state.winners[3].id) {
                    return this.$store.state.winners[2];
                }
